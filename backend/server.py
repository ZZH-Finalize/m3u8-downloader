...
from models import DownloadArgs, ErrorResponse
...
async def download():
    ...
    try:
        ...
        return jsonify(ErrorResponse(msg='参数格式错误').model_dump_json()), 400

    if not param.url:
        return jsonify(ErrorResponse(msg='url无效').model_dump_json()), 400
...
async def list_tasks():
    """列出所有任务 ID 及下载进度"""
    return task.list().unpack()


@app.route('/api/tasks/<task_id>', methods=['GET'])
async def get_task_status(task_id: str):
    """获取任务状态"""
    return task.get(task_id).unpack()


@app.route('/api/tasks/<task_id>/pause', methods=['POST'])
async def pause_task(task_id: str):
    """暂停任务"""
    return task.pause(task_id).unpack()


@app.route('/api/tasks/<task_id>/resume', methods=['POST'])
async def resume_task(task_id: str):
    """恢复任务"""
    return task.resume(task_id).unpack()


@app.route('/api/tasks/<task_id>', methods=['DELETE'])
async def delete_task(task_id: str):
    """删除任务"""
    return task.delete(task_id).unpack()
...
async def cache_get(cache_id: str):
    """获取指定缓存的详细信息"""
    response = await cache.get(cache_id)
    return response.unpack()
...
async def cache_delete(cache_id: str):
    ...
    response = await cache.delete(cache_id)
    return response.unpack()
...
API 端点:
  ...
  DELETE /api/tasks/<id>        - 删除任务（运行中则先取消再删除，已结束则直接删除）
    ...
    parser.add_argument(
        "--cache-dir",
        type=str,
        default=config.server.cache_dir,
        metavar="DIR",
        help=f"缓存目录 (默认：{config.server.cache_dir})"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
...
async def main():
    ...
    # 创建临时目录
    Path(config.server.cache_dir).mkdir(parents=True, exist_ok=True)
    # 创建输出目录
    ...
    logging.info(f"临时文件目录: {config.server.cache_dir}")
