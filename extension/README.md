...
- ⚙️ **灵活配置**：支持自定义服务器地址、端口、并发数、编码格式和编码器类型
- 🔔 **通知提醒**：操作结果通过气泡提示和系统通知反馈
- ⏸️ **任务控制**：支持暂停、恢复、重试和取消任务
...
5. **暂停任务**：点击任务卡片上的 ⏸️ 按钮暂停正在执行的任务
6. **恢复任务**：点击已暂停任务卡片上的 ▶️ 按钮恢复任务
7. **重试失败任务**：失败任务卡片上显示"🔄"按钮，点击即可重新提交
8. **删除任务**：点击任务卡片上的 🗑️ 按钮删除任务
...
- **队列下载**：勾选后将任务加入队列，排队等待执行
- **输出编码**：选择视频编码格式（可选值：`copy`/`h264`/`hevc`/`av1`）
...
| 默认输出编码 | 添加任务时的默认输出编码格式 | copy |
| 默认编码器 | 添加任务时的默认编码器类型 | software |
...
- 配置自动保存，下次打开插件时自动应用
- **编码器选择**：关于编码器的详细说明和选择建议，请参考 [编码器选择指南](ENCODER_GUIDE.md)。**注意**：使用 Docker 镜像后端时，由于镜像内编码器组件已全部移除，编码器设置将被忽略，系统仅支持 copy 模式
...
3. **暂停/恢复任务**：点击"暂停任务"或"恢复任务"按钮控制任务执行
4. **查看详情**：所有信息均为只读，用于了解任务状态
...
- **下载 m3u8**：立即创建下载任务
- **下载 m3u8（队列模式）**：将任务加入队列，等待空闲时执行

#### 操作步骤

1. 在网页上找到视频、音频或 m3u8 链接
2. 右键点击该元素
3. 在菜单中选择"下载 m3u8"或"下载 m3u8（队列模式）"
4. 系统自动创建下载任务并显示通知
...
| `tabs` | 与网页标签通信，发送下载指令 |
| `contextMenus` | 创建右键菜单 |
| `host_permissions` | 访问后端 API 服务（支持 HTTP 和 HTTPS） |
...
| 端口 | 6900 | 后端服务端口 |
| 默认并发数 | 8 | 下载任务默认并发数 |
| 默认输出编码 | copy | 输出编码格式（copy/h264/hevc/av1）。**注意**：使用 Docker 镜像后端时，由于镜像内编码器组件已全部移除，仅支持 copy 模式 |
| 默认编码器 | software | 编码器类型（software/nvenc/qsv/amf/vaapi/mf/d3d12va/vulkan）。**注意**：使用 Docker 镜像后端时，由于镜像内编码器组件已全部移除，此设置将被忽略 |
